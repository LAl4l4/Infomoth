...
describe('SentimentTab', () => {
  it('shows a positive reading for an optimistic score', async () => {
    mockPull.mockResolvedValue({ normalizedScore: 0.3, rollingAverage: 0.2 });
    ...
    expect(screen.getByText('累计滚动平均')).toBeInTheDocument();
  ...
  it('shows a negative reading for a pessimistic score', async () => {
    mockPull.mockResolvedValue({ normalizedScore: -0.2, rollingAverage: -0.1 });
    renderWithProviders(<SentimentTab />);
  ...
  it('shows a neutral reading for a score near zero', async () => {
    mockPull.mockResolvedValue({ normalizedScore: 0.01, rollingAverage: 0.02 });
    renderWithProviders(<SentimentTab />);
